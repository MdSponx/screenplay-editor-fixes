...
const SignIn: React.FC = () => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      const persistenceType = rememberMe ? 'remember' : 'session';
      const result = await signIn(email, password, persistenceType);
      
      if (result.success) {
        window.location.href = '/dashboard';
      } else {
        setError(result.message || t('auth.error.invalidCredentials'));
        ...
        <main className="min-h-screen flex items-center justify-center px-4 py-20 relative z-10">
          <div className="w-full max-w-md">
            ...
            <div className="bg-[#F5F5F2] rounded-xl p-8 shadow-lg relative">
              <h2 className="text-2xl font-semibold text-[#1E4D3A] mb-6">{t('auth.signIn')}</h2>
              {error && (
              ...
              <form className="space-y-4" onSubmit={handleSignIn}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    type="button"
                    disabled={isProcessing}
                    className="flex items-center justify-center py-2.5 px-4 rounded-full border border-gray-200 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed relative z-10"
                  >
